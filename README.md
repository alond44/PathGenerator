...
* dsm: the dsm map we have created at step 1. Note that we can pass a ```python None`` value and load the map using ```python init_map``` method after generating an instance.
...
* start_location: The start location of the drone. If ```python None``` is passed a random one will be generated. 
