...
class PathGenerator:
    ...
    def calc_path_travel_time(self, path: list):
        ...
        return 0
    """
    def __gen_path_probability_new(self, start_location: list, max_cost: float, cost_per_meter: float):
    ...
    def __gen_path_probability(self, start_location: list, max_cost: float, cost_per_meter: float):
        ...
        while True:
            ...
            if self.__is_stride_legal(new_pos_option, cur_pos) and random.randrange(0, 100) <= 96:
                ...
                # path_points += [(new_pos_option, strides[0:last_move] + strides[last_move+1:])]
                ...
                strides.remove(step)
                # del strides[last_move]  # 'del list[i]' deletes the item in the i'th index in list - cool.
                if len(strides) != 0:
                    step = strides[random.randrange(len(strides))]
    ...
    def __is_stride_legal(self, cur_pos, prev_pos):
        ...
        for idx in obstacle_indices:
            if self._obstacle_list[idx].line_intersect(cur_pos, prev_pos):
                return False
        return True
    ...
    def _get_possible_strides(self, cur_pos, prev_pos=None):
        if prev_pos is None:
            prev_pos = [random.randrange(2000) - 1000, random.randrange(2000) - 1000]
        ...
        degrees1 = [cur_deg + (self._max_angle * i / self.DEGREE_DIVISOR) for i in range(1, self.DEGREE_DIVISOR + 1)]
        degrees2 = [cur_deg - (self._max_angle * i / self.DEGREE_DIVISOR) for i in range(1, self.DEGREE_DIVISOR + 1)]
